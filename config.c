...
{
	g_nFontSize = (int)SendDlgItemMessage(hwnd, IDC_SLIDER4, TBM_GETPOS,0, 0);
...
INT_PTR CALLBACK ScreenSaverConfigureDialog(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDOK:
			...
			items = (int)min(MAX_MESSAGES, SendMessage(hwndCtrl, CB_GETCOUNT, 0, 0));
...
int Configure(HWND hwndParent)
{
	...
	DialogBox(GetModuleHandle(0), MAKEINTRESOURCE(IDD_CONFIG), hwndParent, ScreenSaverConfigureDialog);
